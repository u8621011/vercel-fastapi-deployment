...
def get_openapi():
    with open(".well-known/openapi.json", encoding="utf-8") as file:
        return json.loads(file.read())
...
async def manifest_ai_plugin(request: Request):
    ai_plugin = get_ai_plugin()
    return Response(
...
@wellknown.get("/openapi.json", include_in_schema=False)
async def manifest_openapi(request: Request):
    openapi_json = get_openapi()
    return Response(
        content=Template(json.dumps(openapi_json)).substitute(host=get_host(request)),
        media_type="application/json",
    )
